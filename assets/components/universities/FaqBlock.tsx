...
// imports
import { UniversityFaqType } from '@/assets/types/universityTypes';
...
export default function FaqBlock({ faqData }: { faqData: UniversityFaqType[] }) {
            ...
            {faqObjectList.map((faq: FlatFaqType) => {
                return (
                    <div key={faq.id} className="overflow-hidden rounded-xl border border-sky-400">
                        <div
                            className="flex cursor-pointer items-center justify-between bg-sky-100 py-2 pr-2 max-md:items-start"
                            onClick={() =>
                                setShowAnswer((prev) => {
                        ...
                        >
                            <p className="pl-2 select-none max-md:text-left">
                        ...
                        {showAnswer === faq.id && (
                            <div className="bg-white px-4 py-4">
                                <p className="flex items-start justify-start gap-1 max-md:text-left">
