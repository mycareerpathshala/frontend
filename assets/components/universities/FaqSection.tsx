...
export default function FaqSection({
    ...
    initialTopic: UniversityFaqType['selectTopic'];
}) {
    // topics are: ['University' | 'Course' | 'Campus' | 'Cost' | 'Admission']
    const [topic, setTopic] = useState<UniversityFaqType['selectTopic']>(initialTopic);
        ...
        <div id="faq" className="mt-12 rounded-lg border border-gray-200 p-6 max-sm:px-2">
            <div>
                ...
                <div className="mt-4 flex items-center gap-4 overflow-x-auto pb-2">
                    {(['University', 'Course', 'Campus', 'Cost', 'Admission'] as const).map(
                        (item: UniversityFaqType['selectTopic'], index: number) => {
                            return (
                                <button
                                    key={index}
                                    type="button"
